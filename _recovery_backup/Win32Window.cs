...
namespace WinMan.Windows
{
    ...
    public class Win32Window : IWindow
    {
        ...
        public bool CanMove => UseDefaults(() =>
        {
            if (CanAccess())
            {
                return true;
            ...
            if (!IsWindowVisible(new(m_hwnd)))
            {
                return false;
            }

            var style = GetWINDOWS_STYLE(m_hwnd);
            if (style.HasFlag(WINDOWS_STYLE.WS_DISABLED))
            {
                return false;
            }

            return IsLegacyVisualBasicTopLevelWindow(style, GetClassName(m_hwnd));
        }, false);
        ...
        private bool ProbeAccess()
        {
            ...
            var hProcess = OpenProcess(PROCESS_ACCESS_RIGHTS.PROCESS_QUERY_LIMITED_INFORMATION, false, processId);
            if (hProcess == IntPtr.Zero)
            {
                hProcess = OpenProcess(PROCESS_ACCESS_RIGHTS.PROCESS_QUERY_INFORMATION, false, processId);
            }
            if (hProcess == IntPtr.Zero)
            {
                return false;
        ...
        private static bool IsLegacyVisualBasicTopLevelWindow(WINDOWS_STYLE style, string className)
        {
            ...
            if (className.StartsWith("Thunder", StringComparison.OrdinalIgnoreCase))
            {
                // Hidden owner windows (e.g. ThunderRT6Main) are popups without a caption.
                if (style.HasFlag(WINDOWS_STYLE.WS_POPUP)
                    && !style.HasFlag(WINDOWS_STYLE.WS_CAPTION))
                {
                    return false;
                }
            ...
            return className.Equals("wndclass_desked_gsk", StringComparison.OrdinalIgnoreCase);
        }
        ...
        internal static bool GetIsTopLevelVisible(IWorkspace workspace, IntPtr hwnd)
        {
            try
            {
                ...
                if (IsLegacyVisualBasicTopLevelWindow(style, className))
                {
                    return CheckCloaked();
                }
        ...
        private void UpdateConfiguration()
        {
            ...
            try
            {
                ...
                if (state == WindowState.Restored
                    && m_workspace.DisplayManager.Displays.Any(x => CloseMatch(x.Bounds, placement, threshold: 2))
                    && !IsLegacyVisualBasicTopLevelWindow(GetWINDOWS_STYLE(m_hwnd), GetClassName(m_hwnd)))
                {
