...
echo ========================================
echo FancyWM AutoBuild - Framework-Dependent
echo ========================================
echo Timestamp : %BUILD_TIMESTAMP%
echo Output    : %OUTPUT_DIR%
echo SDK       : Windows %WINDOWS_SDK_VERSION%
echo Runtime   : uses system .NET (smaller output)
echo.
...
echo [3/4] Publishing framework-dependent Release...
if not exist "%OUTPUT_DIR%" mkdir "%OUTPUT_DIR%"

dotnet publish "FancyWM.GUI\FancyWM.GUI.csproj" -c Release -r %RUNTIME_IDENTIFIER% --self-contained false -o "%OUTPUT_DIR%" --no-restore
if errorlevel 1 goto :error
